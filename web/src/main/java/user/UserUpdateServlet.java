...
public class UserUpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (userID == null || userID.equals("") || userPW == null || userPW.equals("") || userPWC == null || userPWC.equals("") 
				|| userName == null || userName.equals("") || userBirthYear == null
				|| userBirthYear.equals("") || userBirthMonth == null || userBirthMonth.equals("")
				|| userBirthDate == null || userBirthDate.equals("") || userEmail == null || userEmail.equals("")
		...
		if (file != null) {
			...
			if (ext.equals("jpg") || ext.equals("png") || ext.equals("gif")) {
				...
				File prevFile = new File(savePath + "/" + prev); // 사용자의 이전 프로필사진 경로 찾기
				if (prevFile.exists()) { // 사용자의 이전 프로필 사진이 있다면
					prevFile.delete(); // 사용자의 이전 프로필 사진 삭제
				}
				fileName = file.getName(); // 파일의 실제 이름을 사용자의 새로운 프로필 사진으로 저장
			} else {
				if (file.exists()) {
					file.delete();
				}
		...
		new UserDAO().profile(userID, fileName); // 사용자의 프로필사진 파일을 DB에 업데이트
		int result = new UserDAO().update(userID, userPW, userName, userBirthYear, userBirthMonth, userBirthDate,
				userEmail, userGender, userProfile);
		if (result == 1) {
